...
sealed class StravaError {
    ...
    // 400 Bad request
    class BadRequest(response: StravaErrorResponse): Error("Bad request, response: '${response.description}'")
    ...
    class Forbidden(response: StravaErrorResponse): Error("Forbidden, response: '${response.description}'")

    // 4XX Other
    class ApiUnexpectedError(response: StravaErrorResponse): Error("Unexpected error, response: '${response.description}'")
}
